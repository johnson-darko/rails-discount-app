class WishesController < ApplicationController
  rescue_from ActiveRecord::RecordNotFound, with: :invalid_wish
  ...
  def destroy
    @wish.destroy if @wish.id == session[:wish_id]
    session[:wish_id] = nil
    respond_to do |format|
      format.html { redirect_to root_path, notice: 'Wish was successfully destroyed.' }
  ...
  private
    ...
    def invalid_wish
      logger.error "Attempt to access invalid wish #{params[:id]}"
      redirect_to root_path, notice: "That wish doesn't exist"
    end
end
